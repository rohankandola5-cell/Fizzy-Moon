  ...
  return (
    <div className="fixed bottom-4 left-4 md:bottom-6 md:left-6 z-50 pointer-events-auto">
      ...
      <motion.button
        ...
        className={`h-12 md:h-14 min-w-[3rem] md:min-w-[3.5rem] bg-black/40 backdrop-blur-md border border-white/10 rounded-full shadow-lg flex items-center justify-center overflow-hidden group transition-all duration-300 ${isPlaying ? 'border-[#f59e0b]/50 shadow-[#f59e0b]/20' : 'hover:border-white/30'}`}
      >
        <div className="flex items-center">
           <div className={`w-12 md:w-14 h-12 md:h-14 flex items-center justify-center shrink-0 relative z-10`}>
             {isPlaying ? (
               <Volume2 className="w-5 h-5 md:w-6 md:h-6 text-[#f59e0b] animate-pulse" />
             ) : (
