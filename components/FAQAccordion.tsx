    ...
    <div className="w-full space-y-4">
      {faqs.map((faq, index) => {
        ...
        return (
          <div 
            key={index} 
            className={`border rounded-2xl transition-all duration-300 ${isOpen ? 'bg-white/10 border-[#f59e0b]/50' : 'bg-white/5 border-white/10 hover:border-white/20'}`}
          >
            <button
              onClick={() => toggleIndex(index)}
              aria-expanded={isOpen}
              aria-controls={`faq-answer-${index}`}
              className="w-full flex items-center justify-between p-6 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-[#f59e0b] rounded-2xl group"
            >
              <span className={`text-lg md:text-xl font-bold font-heading uppercase tracking-wide transition-colors ${isOpen ? 'text-[#f59e0b]' : 'text-white group-hover:text-white/90'}`}>
                {faq.question}
              </span>
              <span className={`p-2 rounded-full border transition-all ${isOpen ? 'bg-[#f59e0b] border-[#f59e0b] text-black rotate-180' : 'border-white/20 text-white group-hover:border-white/40'}`}>
